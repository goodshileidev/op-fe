import React, {useEffect, useState} from 'react'
import {Space,Modal, Popconfirm, Table, Tag, TableProps, Button, Card, message, Input, Spin} from 'antd'
import {DeleteFilled, EditFilled} from '@ant-design/icons'
import {ReceiptOrgType} from '@/common/data_type/system/receipt_org'
import {Link} from 'umi'
import {useTranslation} from 'react-i18next';
import ReceiptOrgList from './receipt_org_list'

interface IReceiptOrgListModalProps {
  isReceiptOrgListModalOpen?: boolean

  onClose: any
}

const handleClose = () => {
    props.onClose(true)
    return true
}

const ReceiptOrgListModal: React.FC<IReceiptOrgListModalProps> = Observer((props) => {
  const [isModalOpen, setIsModalOpen] = useState(false);
  let isOpen = props.isReceiptOrgListModalOpen
  const { t } = useTranslation();

  return (
      <div>
        <Modal
          width={"80%"}
          title={t("common.title.detail",{'entity':t('system.receipt_org')})}
          open={isOpen}
          okText={t("common.close")}
          onOk={handleClose}
          okType={"primary"}
          // footer={[<Button type={'primary'} onClick={() => {
          //   isOpen = false
          //   console.debug("isOpen", isOpen)
          // }
          // }></Button>,
          //   <Button onClick={() => {
          //     isOpen = false
          //     console.debug("isOpen", isOpen)
          //   }
          //   }>
          //   </Button>]}
        >
        <ReceiptOrgList  />
        </Modal>
      </div>
    )
})

export default ReceiptOrgListModal;
